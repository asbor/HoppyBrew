...
async def get_style_guideline_source(
    source_id: int,
    db: Session = Depends(get_db)
):
    """Get details of a specific style guideline source."""
    source = db.query(models.StyleGuidelineSource).filter(
        models.StyleGuidelineSource.id == source_id
    ).first()
    if not source:
...
async def create_style_guideline_source(
    source: schemas.StyleGuidelineSourceCreate,
    db: Session = Depends(get_db)
):
...
async def update_style_guideline_source(
    ...
    db_source = db.query(models.StyleGuidelineSource).filter(
        models.StyleGuidelineSource.id == source_id
    ).first()
    if not db_source:
        raise HTTPException(status_code=404, detail="Style guideline source not found")
...
@router.delete(
    "/style-guideline-sources/{source_id}",
    summary="Delete a style guideline source"
)
async def delete_style_guideline_source(
    source_id: int,
    db: Session = Depends(get_db)
):
...
async def create_style_category(
    category: schemas.StyleCategoryCreate,
    db: Session = Depends(get_db)
):
...
async def update_style_category(
    ...
    db_category = db.query(models.StyleCategory).filter(
        models.StyleCategory.id == category_id
    ).first()
    if not db_category:
        raise HTTPException(status_code=404, detail="Style category not found")
...
@router.delete(
    "/style-categories/{category_id}",
    summary="Delete a style category"
)
...
async def get_beer_style(
    style_id: int,
    db: Session = Depends(get_db)
):
    """Get details of a specific beer style."""
    style = db.query(models.BeerStyle).options(
        joinedload(models.BeerStyle.guideline_source),
        joinedload(models.BeerStyle.category)
    ).filter(models.BeerStyle.id == style_id).first()
...
async def update_beer_style(
    style_id: int,
    style: schemas.BeerStyleUpdate,
    db: Session = Depends(get_db)
):
    """Update an existing beer style. Only custom styles can be updated."""
    db_style = db.query(models.BeerStyle).filter(
        models.BeerStyle.id == style_id
    ).first()
...
async def delete_beer_style(
    ...
    if not db_style.is_custom:
        raise HTTPException(
            status_code=403, 
            detail="Cannot delete standard beer styles"
        )
