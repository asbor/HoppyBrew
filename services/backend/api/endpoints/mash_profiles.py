...
async def create_mash_profile(
    profile: schemas.MashProfileBase,
    db: Session = Depends(get_db)
):
    """
    ...
    existing = db.query(models.MashProfiles).filter(
        models.MashProfiles.name == profile.name
    ).first()
...
async def get_mash_profile(
    ...
    """
    profile = db.query(models.MashProfiles).filter(
...
async def get_mash_steps(
    ...
    steps = db.query(models.MashStep).filter(
        models.MashStep.mash_id == profile_id
    ).order_by(models.MashStep.id).all()
...
async def update_mash_step(
    step_id: int,
    step_update: schemas.MashStepBase,
    db: Session = Depends(get_db)
):
    """
