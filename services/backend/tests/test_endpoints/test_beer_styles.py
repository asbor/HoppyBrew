...
def test_update_style_guideline_source(client, db_session):
    ...
    response = client.put(
        f"/style-guideline-sources/{source.id}",
...
def test_get_style_categories_by_guideline(client, db_session):
    ...
    cat1 = models.StyleCategory(
        guideline_source_id=source1.id,
        name="American Ale",
        code="18"
    )
    cat2 = models.StyleCategory(
        guideline_source_id=source2.id,
        name="American IPA",
        code="21"
    )
...
def test_update_custom_beer_style(client, db_session):
    ...
    response = client.put(
        f"/beer-styles/{style.id}",
        json={"description": "My special custom IPA recipe"}
    )
    assert response.status_code == 200
    data = response.json()
