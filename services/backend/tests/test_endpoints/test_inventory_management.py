...
def create_recipe_with_ingredients(client, db_session):
    ...
    recipe = (
        ...
        .filter(models.Recipes.name == "Test Recipe for Inventory")
        .one()
    )
    return recipe.id
...
def create_batch_with_inventory(client, db_session):
    ...
    inventory_hops = (
        db_session.query(models.InventoryHop)
        .filter(models.InventoryHop.batch_id == batch["id"])
        .all()
    )
    inventory_fermentables = (
        db_session.query(models.InventoryFermentable)
        .filter(models.InventoryFermentable.batch_id == batch["id"])
        .all()
    )
...
def test_consume_ingredients_success(client, db_session):
    ...
    batch_id, inventory_hops, inventory_fermentables = create_batch_with_inventory(
        client, db_session
    )
    ...
    response = client.post(
        f"/batches/{batch_id}/consume-ingredients", json=consume_request
    )
    assert response.status_code == 200, response.text
    ...
    batch_ingredients = (
        db_session.query(models.BatchIngredient)
        .filter(models.BatchIngredient.batch_id == batch_id)
        .all()
    )
    ...
    transactions = (
        db_session.query(models.InventoryTransaction)
        .filter(
