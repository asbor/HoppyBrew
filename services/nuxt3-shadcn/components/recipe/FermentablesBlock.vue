<template>
  <div class="bg-white rounded-lg border p-6 shadow-sm">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-semibold text-gray-900">Fermentables</h3>
      <span class="text-sm text-gray-500">{{ fermentables.length }} item{{ fermentables.length !== 1 ? 's' : '' }}</span>
    </div>
    ...
    <div class="space-y-3">
      ...
      >
        ...
        <div class="text-right">
          ...
          <div v-if="fermentable.percentage" class="text-sm text-gray-500">
    ...
    <div class="mt-4 pt-4 border-t border-gray-200">
      <div class="flex justify-between text-sm">
        <span class="text-gray-500">Total Fermentables:</span>
        <span class="font-medium">{{ totalAmount.toFixed(2) }} kg</span>
      </div>
    </div>
  </div>
</template>
