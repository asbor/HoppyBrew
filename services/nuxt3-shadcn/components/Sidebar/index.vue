...
<template>
  <div>
    <div class="z-50 flex items-center justify-between w-full h-full p-4 lg:hidden">
      ...
      <div>
        <Icon
class="z-50 cursor-pointer left-2 top-2" size="30" name="iconamoon:menu-burger-horizontal"
          @click="open = true" />
      </div>

      <div v-if="open" name="overlay" class="bg-background fixed top-0 left-0 z-[999] w-full h-full h-screen">
        <Icon
class="absolute z-50 cursor-pointer right-4 top-4" size="30" name="material-symbols:close"
          @click="open = false" />
        <SidebarMenu @link-clicked="closeOverlay" />
      </div>
    </div>
