...
const stats = computed(() => {
  ...
  return {
    ...
    lastBrewDate: batches.value.length > 0
      ? [...batches.value]
          .sort((a, b) => new Date(b.brew_date || b.created_at).getTime() - new Date(a.brew_date || a.created_at).getTime())[0]
          .brew_date
      : null,
  }
})
    ...
    <main v-else class="grid w-full gap-ha-md">
      ...
      <div class="grid gap-ha-sm md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div
              class="text-ha-2xl font-bold"
              :class="{ 'text-orange-500': stats.lowStockCount > 0 }"
            >
              {{ stats.lowStockCount }}
            </div>
            <p class="text-ha-xs text-muted-foreground">
